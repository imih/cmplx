...
template <class T> IVector<T>::IVector(std::initializer_list<T> il) {
...
template <class T> T IVector<T>::operator[](int idx) const {
  assert(idx >= 0);
  assert(idx < (int)size());
  return (T)VECTOR(vector_)[idx];
}

template <class T> void IVector<T>::set(int idx, T val) {
  assert(idx >= 0);
  assert(idx < (int)size());
  VECTOR(vector_)[idx] = val;
}
