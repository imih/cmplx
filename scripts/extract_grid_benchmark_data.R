...
source("pnas-sis/cmplx/scripts/entropy.R")
...
readRealization <- function(id) {
  ...
  return(c(true_source, p, q, T, realization_size))
}
...
getBenchmarkRow <- function(row_id, result_prefix) {
  ...
  data_sol <- read.table(paste(
    "~/dipl/Supplementary_data_code/Data/benchmark_data/solutions/inverse_solution_", 
    ben_id, ".txt", sep = ""), 
    header = FALSE, 
    sep = "\n",  
    stringsAsFactors = FALSE, 
  ...
  P_estimated_numeric = 
    as.numeric(strsplit(P_estimated, split = " ")[[1]])
  estimated_true_rank = 
    rank(-P_estimated_numeric, ties.method = "first")[true_source + 1]
  estimated_MAP_rank = 
    rank(-P_estimated_numeric, ties.method = "first")[MC_MAP + 1]
